...
SELECT *
FROM service
WHERE service_id IN (SELECT service_id FROM Employee_Service es WHERE es.employee_id = 1);

-- NOT IN 
SELECT *
FROM employee e
WHERE e.employee_id NOT IN (SELECT employee_id FROM Employee_Service es WHERE es.service_id = 1);

-- EXISTS 
SELECT *
FROM services
WHERE EXISTS (SELECT 1 FROM Employee_Serviceces WHERE es.service_id = s.service_id);
